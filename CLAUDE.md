...
- `scripts/build.sh` builds `dist/vinted-country-filter-<version>.zip` from committed files at HEAD. It leaves out repo-only files (`store/`, `scripts/`, docs) and fails if the manifest references a file that isn't committed. This zip is what gets uploaded to the Chrome Web Store.
...
- `enabled: false` makes `apply()` hand off to `applyOff()`, which removes badges and dim/hide classes, and makes `pump()` start no new lookups. The queue is kept, so lookups resume when the filter is turned back on.
